...
#
# Writes: dist/Vigilante-8-Recomp-<VERSION>-<artifact-tag>.zip
set -euo pipefail
...
exec bash "${PACKAGER}" \
  ...
  --zip-prefix Vigilante-8-Recomp \
  --exe-name Vigilante_8_Recompiled \
  --display-name "Vigilante 8 Recompiled" \
  --recompiler-build "${RECOMPILER_BUILD}" \
  --version-env RELEASE_VERSION \
  --disc-hint "your legally owned Vigilante 8 disc" \
